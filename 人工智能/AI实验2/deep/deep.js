...
function deepFirstSearch() {    //深度优先算法
    ...
    while (openList.length > 0) {
        if (res.length >= maxLen) { //若到达了设置的最大深度，则回退
            res.pop();
            for (let i = 0; i < addCount; i++) {
                let t = openList.pop();
                console.log('open表删除[' + t.statu + ']结点');
            }
            //扩展点要在res最后一个元素的下一层
            while (openList[openList.length - 1].level <= res.length)
                res.pop();
            continue;
        }
        expandNode = openList.pop();    //取出open表的第一个结点
        console.log('open表取出[' + expandNode.statu + ']结点');
        closeList.push(expandNode);     //把取出的结点放入close表
        console.log('close表添加[' + expandNode.statu + ']结点');
        console.log('--------------------------------------------');
        res.push(expandNode);   //放入路径数组
        ...
        for (let direction of optionOfBlock[expandNode.pos]) {  //开始扩展
            ...
            let tempNode = new StatuNode(curIndex, res.length + 1, [...tempStatu]);
            openList.push(tempNode);    //加入open表前头
